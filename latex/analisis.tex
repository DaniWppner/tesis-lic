...
Para responder estas preguntas analizamos el desempeño de los prototipos implementados usando ambos algoritmos sobre dos algunos casos de prueba.
Aquí veremos la evaluación para dos contratos inteligentes.

\subsection{Caso \texttt{RoomThermostat}}
El primer caso que evaluamos fue el contrato \texttt{RoomThermostat} perteneciente al benchmark ``Microsoft Azure Blockchain Workbench" \cite{azure-benchmark}.

\subsection{Caso \texttt{BoundedStack}}
El primer caso que evaluamos fue el ejemplo teórico que expusimos en la sección \ref{sec:subaproximacion}, el contrato \texttt{BoundedStack} (cuyo código se encuentra en \ref{code:solidity-bounded-stack}).
Queremos corroborar cuál es el comportamiento de la implementación producida, evaluar si se produce alguna subaproximación, y cuáles.

Lo primero que vimos al evaluar la abstracción generada por el prototipo sobre el contrato \texttt{BoundedStack} lo podemos ver en la imagen \ref{fig:buggy-bounded-stack-epa}.
